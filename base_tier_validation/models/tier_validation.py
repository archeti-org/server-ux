...
class TierValidation(models.AbstractModel):
    ...
    def _validate_tier(self, tiers=False):
        ...
        notify_creator = any(
            tier_reviews.filtered(
                lambda r: r.definition_id.notify_creator_full_validated
            )
        )
    ...
    def _notify_review_accepted_requested_by_sequence(self, tier_review):
        if hasattr(self, "message_pos") and hasattr(self, "message_subscribe"):
            for rec in self:
                getattr(rec, "message_subscribe")(
                    partner_ids=tier_review.reviewer_ids.mapped("partner_id").ids
                )
    ...
    def request_validation(self):
        ...
        if (
            created_trs
            and not created_trs[0].definition_id.notify_on_create
            and created_trs[0].definition_id.notify_by_sequence
        ):
